...
from . import metrics_generator, monitoring_utility
from .configuration_controller import generate_deployment_files
from .metrics_generator import get_new_filetag, get_total_files_length, get_latest_filetag
from .deployment_file_templates import get_initial_configuration_template
...
def get_initial_files(dir_path, initial_template):
    ...
    configfile = str(get_new_filetag(dir_path)) + 'config.json'
    # generate the name for new data file
    datafile = str(get_new_filetag(dir_path)) + 'data.json'
    # populate new configuration file
    metrics_generator.generate_config_file(initial_template, dir_path, configfile)
    # populate new data file
    metrics_generator.generate_data(dir_path, configfile, datafile)
...
def get_latest_data_object(dir_path, config_file, data_file):
    ...
    metrics_generator.generate_data(dir_path, config_file, data_file, pod_cpu=pod_cpu, pod_ram=pod_ram,
...
def data_monitor(paths, service_ports):
    ...
    for run in range(50):
        ...
        data_read_file = str(get_latest_filetag(dir_path)) + 'data.json'
        ...
        config_read_file = str(get_latest_filetag(dir_path)) + 'config.json'
        ...
        new_template = monitoring_utility.monitor_data(dataset, config_read_path)
        ...
        if not new_template:
            ...
            data_file = str(get_latest_filetag(dir_path)) + 'data.json'
            config_file = str(get_latest_filetag(dir_path)) + 'config.json'
            ...
            new_config_file = str(get_new_filetag(dir_path)) + 'config.json'

            # creating name for new data file
            new_data_file = str(get_new_filetag(dir_path)) + 'data.json'

            # creating config.json file with new template
            metrics_generator.generate_config_file(new_template, dir_path, new_config_file)

            # creating the data.json file populate data according to new template
            metrics_generator.generate_data(dir_path, new_config_file, new_data_file)
            ...
            generate_deployment_files(metrics_generator.get_latest_filetag(dir_path), paths)
